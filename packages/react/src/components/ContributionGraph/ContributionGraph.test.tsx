  ...
  it('updates accent contribution colours with GnomeProvider', () => {
    ...
    const { container, rerender } = render(
      <GnomeProvider colorScheme="light" accentColor="purple">
        <ContributionGraph data={data} responsive={false} />
      </GnomeProvider>,
    );

    expect(container.querySelector('rect[fill*="--gnome-purple-"]')).toBeInTheDocument();
    ...
    expect(container.querySelector('rect[fill*="--gnome-red-"]')).toBeInTheDocument();
  });

  it('uses the default named accent tokens without a provider', () => {
    const { container } = render(
      <ContributionGraph data={[{ date: todayIso(), count: 3 }]} responsive={false} />,
    );

    expect(container.querySelector('rect[fill*="--gnome-blue-"]')).toBeInTheDocument();
  });
});
