...
function getShortMonth(monthIndex: number, formatter: Intl.DateTimeFormat): string {
  return formatter.format(new Date(2000, monthIndex));
}

function getShortDay(dayIndex: number, formatter: Intl.DateTimeFormat): string {
...
export function ContributionGraph({
  ...
  const { grid, monthLabels } = useMemo(() => {
    ...
    for (let col = 0; col < visibleWeeks; col++) {
      ...
      for (let row = 0; row < 7; row++) {
        ...
        const level =
          count === 0 ? 0 : Math.min(maxLevel, Math.ceil((count / maxCount) * maxLevel));
  ...
  function focusCell(col: number, row: number) {
    ...
    svgRef.current?.querySelector<SVGRectElement>(`[data-col="${c}"][data-row="${r}"]`)?.focus();
  }
  ...
  function handleCellKey(e: KeyboardEvent, col: number, row: number) {
    ...
    if (moves[e.key]) {
      ...
      if (cell && !cell.future) {
        onDayClick?.({ date: cell.iso, count: cell.count });
      }
    }
  }
  ...
  function cellTooltip(cell: GridCell): string {
    if (cell.future) {
      return fullDateLabel(cell.iso, fullDateFormatter);
    }
        ...
        >
          ...
          {showMonthLabels &&
            monthLabels.map(({ col, month }) => (
              <text key={`m-${col}`} x={dayLabelW + col * stride} y={12} className={styles.label}>
                {month}
              </text>
            ))}
                ...
                {column.map((cell, row) => {
                  ...
                  return (
                    <rect
                      ...
                      onClick={() =>
                        !cell.future && onDayClick?.({ date: cell.iso, count: cell.count })
                      }
