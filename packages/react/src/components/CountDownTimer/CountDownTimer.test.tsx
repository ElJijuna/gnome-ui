...
describe('CountDownTimer', () => {
  ...
  describe('countdown behavior', () => {
    ...
    it('applies finished class when end date is in the past', () => {
      const pastDate = new Date(Date.now() - 1000);
      const { container } = render(<CountDownTimer start={new Date()} end={pastDate} />);
    ...
    it('switches to destructive variant when countdown finishes', () => {
      const pastDate = new Date(Date.now() - 1000);
      const { container } = render(
        <CountDownTimer start={new Date()} end={pastDate} variant="accent" />,
      );
  ...
  describe('callback', () => {
    it('calls the action callback when countdown finishes', async () => {
      ...
      render(<CountDownTimer start={new Date()} end={pastDate} action={mockAction} />);
  ...
  describe('edge cases', () => {
    ...
    it('accepts custom className', () => {
      ...
      const { container } = render(
        <CountDownTimer start={new Date()} end={futureDate} className="custom-class" />,
      );
