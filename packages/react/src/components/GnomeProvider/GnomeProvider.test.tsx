...
describe("GnomeProvider colorScheme", () => {
  ...
  it("does not set data-theme when colorScheme is system (default)", () => {
    ...
    render(<GnomeProvider><span /></GnomeProvider>);
    expect(document.documentElement.hasAttribute("data-theme")).toBe(false);
  });

  it("sets data-theme=light when colorScheme is light", () => {
    mockMatchMedia(false);
    render(<GnomeProvider colorScheme="light"><span /></GnomeProvider>);
    expect(document.documentElement.getAttribute("data-theme")).toBe("light");
  });

  it("sets data-theme=dark when colorScheme is dark", () => {
    mockMatchMedia(false);
    render(<GnomeProvider colorScheme="dark"><span /></GnomeProvider>);
  ...
  it("exposes colorScheme via useColorScheme", () => {
    ...
    render(<GnomeProvider colorScheme="dark"><Reader /></GnomeProvider>);
  ...
  it("resolves system to light when OS is light", () => {
    ...
    render(<GnomeProvider colorScheme="system"><Reader /></GnomeProvider>);
...
describe("GnomeProvider accentColor", () => {
  ...
  it("sets correct CSS vars for a named color in light mode", () => {
    mockMatchMedia(false);
    render(<GnomeProvider colorScheme="light" accentColor="green"><span /></GnomeProvider>);
    expect(document.documentElement.style.getPropertyValue("--gnome-accent-color")).toBe("var(--gnome-green-3)");
    expect(document.documentElement.style.getPropertyValue("--gnome-accent-bg-color")).toBe("var(--gnome-green-3)");
  });

  it("sets correct CSS vars for a named color in dark mode (shade-2)", () => {
    mockMatchMedia(false);
    render(<GnomeProvider colorScheme="dark" accentColor="purple"><span /></GnomeProvider>);
    expect(document.documentElement.style.getPropertyValue("--gnome-accent-color")).toBe("var(--gnome-purple-2)");
    expect(document.documentElement.style.getPropertyValue("--gnome-accent-bg-color")).toBe("var(--gnome-purple-3)");
  });

  it("applies hex color directly for custom (non-named) accent", () => {
    mockMatchMedia(false);
    render(<GnomeProvider accentColor="#a020f0"><span /></GnomeProvider>);
    expect(document.documentElement.style.getPropertyValue("--gnome-accent-color")).toBe("#a020f0");
    expect(document.documentElement.style.getPropertyValue("--gnome-accent-bg-color")).toBe("#a020f0");
  });
  ...
  it("exposes accentColor via useAccentColor", () => {
    ...
    render(<GnomeProvider accentColor="red"><Reader /></GnomeProvider>);
    expect(screen.getByText("red")).toBeInTheDocument();
  });

  it("clears inline CSS vars when switching back to blue", () => {
    mockMatchMedia(false);
    const { rerender } = render(<GnomeProvider accentColor="green"><span /></GnomeProvider>);
    expect(document.documentElement.style.getPropertyValue("--gnome-accent-color")).not.toBe("");
    rerender(<GnomeProvider accentColor="blue"><span /></GnomeProvider>);
    expect(document.documentElement.style.getPropertyValue("--gnome-accent-color")).toBe("");
  });
});
