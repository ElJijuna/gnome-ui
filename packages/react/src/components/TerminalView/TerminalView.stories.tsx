...
const LONG_LINES = Array.from(
  { length: 200 },
  (_, i) => `[${String(i + 1).padStart(3, '0')}] Processing file-${i + 1}.ts...`,
);
...
export const WithLongOutput: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          '`maxLines={50}` — only the last 50 of 200 lines are rendered. `autoScroll` scrolls to the bottom on mount.',
      },
    },
  },
};
...
export const AutoScrollDisabled: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          '`autoScroll={false}` — the terminal starts at the top even when many lines are present.',
      },
    },
  },
};
...
export const LiveStreaming: Story = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        story:
          'Lines streamed one at a time — `autoScroll` keeps the view pinned to the latest output.',
      },
    },
  },
};
