  ...
  render: function WithValueStory() {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, maxWidth: 360 }}>
        <Slider aria-label="Brightness" value={value} onChange={setValue} style={{ flex: 1 }} />
        <Text
          variant="body"
          style={{ minWidth: 32, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}
        >
          {value}%
        </Text>
      </div>
  ...
  render: function DecimalStory() {
    ...
    return (
      <div style={{ maxWidth: 340 }}>
        ...
        <Slider aria-label="Opacity" value={value} onChange={setValue} min={0} max={1} step={0.1} />
      </div>
    );
  },
  parameters: {
