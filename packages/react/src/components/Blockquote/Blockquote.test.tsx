...
describe('Blockquote', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders cite inside a <footer><cite> when provided', () => {
      const { container } = render(<Blockquote cite="Ada Lovelace">Some quote</Blockquote>);
    ...
    it('renders icon when provided', () => {
      render(<Blockquote icon={<span data-testid="icon">★</span>}>Text</Blockquote>);
  ...
  describe('variants', () => {
    ...
    it.each([
      ...
      const { container } = render(<Blockquote variant={variant}>Text</Blockquote>);

      expect(container.querySelector('blockquote')).toHaveClass(new RegExp(variant));
    });
  });
  ...
  describe('HTML attribute forwarding', () => {
    ...
    it('forwards inline style to the root element', () => {
      const { container } = render(<Blockquote style={{ maxWidth: '400px' }}>Text</Blockquote>);

      expect((container.querySelector('blockquote') as HTMLElement).style.maxWidth).toBe('400px');
    });

    it('forwards arbitrary HTML attributes to the root element', () => {
      const { container } = render(
        <Blockquote data-testid="bq" aria-label="Quote">
          Text
        </Blockquote>,
      );
