...
describe('ButtonContent', () => {
  ...
  it('renders the icon when provided', () => {
    render(<ButtonContent label="Save" icon={<span data-testid="icon">★</span>} />);
    expect(screen.getByTestId('icon')).toBeInTheDocument();
  });

  it('places icon before label by default (iconPosition=start)', () => {
    const { container } = render(<ButtonContent label="Save" icon={<span data-testid="icon" />} />);
  ...
  it('places icon after label when iconPosition=end', () => {
    const { container } = render(
      <ButtonContent label="Save" icon={<span data-testid="icon" />} iconPosition="end" />,
    );
    const root = container.firstChild as HTMLElement;
    const children = Array.from(root.children);
    const iconIdx = children.findIndex((c) => c.querySelector("[data-testid='icon']"));
