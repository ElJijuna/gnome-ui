...
describe('Timeline', () => {
  ...
  describe('connector lines', () => {
    ...
    it("renders no lines when variant='none'", () => {
      const { container } = render(<Timeline items={basicItems} variant="none" />);

      expect(container.querySelectorAll("[aria-hidden='true']")).toHaveLength(0);
    });

    it("renders lines with dotted class when variant='dotted'", () => {
      const { container } = render(<Timeline items={basicItems} variant="dotted" />);
    ...
    it('single-item list renders no connector line', () => {
      const { container } = render(<Timeline items={[{ content: 'Solo' }]} />);
  ...
  describe('vertical orientation (default)', () => {
    ...
    it('applies vertical class explicitly', () => {
      const { container } = render(<Timeline items={basicItems} orientation="vertical" />);
  ...
  describe('node variants', () => {
    it('applies nodeWithIcon class when icon is provided', () => {
      const { container } = render(
        <Timeline items={[{ icon: <span>★</span>, content: 'With icon' }]} />,
      );
    ...
    it('applies nodeDot class when icon is omitted', () => {
      const { container } = render(<Timeline items={[{ content: 'No icon' }]} />);
  ...
  describe('HTML attribute forwarding', () => {
    it('forwards className to the root element', () => {
      const { container } = render(<Timeline items={basicItems} className="my-timeline" />);

      expect(container.firstChild).toHaveClass('my-timeline');
    });

    it('forwards inline style to the root element', () => {
      const { container } = render(<Timeline items={basicItems} style={{ width: '500px' }} />);
    ...
    it('forwards arbitrary HTML attributes to the root element', () => {
      const { container } = render(
        <Timeline items={basicItems} data-testid="my-timeline" aria-label="Activity timeline" />,
      );
