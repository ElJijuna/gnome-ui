...
describe('Avatar', () => {
  describe('rendering', () => {
    ...
    it('renders an img when src is provided', () => {
      const { container } = render(<Avatar name="Alice" src="https://example.com/alice.jpg" />);
    ...
    it('uses explicit alt text when provided', () => {
      const { container } = render(
        <Avatar name="Alice" src="https://example.com/alice.jpg" alt="Profile photo" />,
      );
      const img = container.querySelector('img');
  ...
  describe('sizes', () => {
    it.each(['sm', 'md', 'lg', 'xl'] as const)('applies %s size class', (size) => {
      ...
      expect(container.querySelector('span')?.className).toMatch(new RegExp(size));
    });
  });
