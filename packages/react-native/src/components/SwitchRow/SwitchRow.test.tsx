...
describe('SwitchRow', () => {
  describe('rendering', () => {
    it('renders title and subtitle', async () => {
      await render(<SwitchRow title="Wi-Fi" subtitle="Home Network" />);

      expect(screen.getByText('Wi-Fi')).toBeOnTheScreen();
      expect(screen.getByText('Home Network')).toBeOnTheScreen();
    });

    it('exposes accessibilityRole="switch"', async () => {
      await render(<SwitchRow title="Wi-Fi" testID="row" />);

      expect(screen.getByTestId('row').props.accessibilityRole).toBe('switch');
    });

    it('renders leading content', async () => {
      await render(<SwitchRow title="Bluetooth" leading={<Text testID="leading">B</Text>} />);

      expect(screen.getByTestId('leading')).toBeOnTheScreen();
    });

    it('combines title and subtitle into accessibilityLabel', async () => {
      await render(<SwitchRow title="Wi-Fi" subtitle="Home Network" testID="row" />);
  ...
  describe('checked state', () => {
    ...
    it('is checked when defaultChecked is true', async () => {
      await render(<SwitchRow title="Wi-Fi" defaultChecked testID="row" />);

      expect(screen.getByTestId('row').props.accessibilityState.checked).toBe(true);
    });

    it('reflects a controlled checked prop', async () => {
      await render(<SwitchRow title="Wi-Fi" checked onCheckedChange={() => {}} testID="row" />);
    ...
    it('does not change on press when controlled', async () => {
      ...
      await render(
        <SwitchRow title="Wi-Fi" checked={false} onCheckedChange={onCheckedChange} testID="row" />,
      );
  ...
  describe('interactions', () => {
    ...
    it('calls onCheckedChange with the next value on press', async () => {
      ...
      await render(<SwitchRow title="Wi-Fi" onCheckedChange={onCheckedChange} testID="row" />);
      await fireEvent.press(screen.getByTestId('row'));
    ...
    it('does not toggle when disabled', async () => {
      ...
      await render(
        <SwitchRow title="Wi-Fi" disabled onCheckedChange={onCheckedChange} testID="row" />,
      );
      await fireEvent.press(screen.getByTestId('row'));
    ...
    it('is disabled when disabled prop is set', async () => {
      await render(<SwitchRow title="Wi-Fi" disabled testID="row" />);
  ...
  it('forwards a ref to the underlying view', async () => {
    ...
    await render(<SwitchRow title="Wi-Fi" ref={ref} />);
