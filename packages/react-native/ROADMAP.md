...
| ✅ | **CalendarRange** | Shipped — start/end range selection on `Calendar`'s own grid engine. **Extracted `CalendarBase.tsx`** (grid navigation, heading, day/month/year drill-down) out of `Calendar.tsx` for the first time, exactly as that component's own note predicted — `Calendar` itself was refactored into a thin single-day `CalendarSelectionModel` over it, with zero behavior change (all 29 pre-existing `Calendar.test.tsx` tests passed unmodified against the refactor, no assertions touched). `CalendarSelectionModel` is trimmed from the web version's own contract: no `seedFocus`/`hoverDay`/`focusDayChange`/`escape` — those exist purely to drive the web keyboard roving-tabindex and mouse-hover range preview, and this package already drops the whole keyboard layer (`Calendar`'s own precedent). **Real design consequence of dropping hover**: without a hover signal, a range picker's "preview" between the two taps has nothing to grow against — the web version paints a live band under the mouse/keyboard focus before the second click, but RN has neither. Rather than reach for a `PanResponder` drag gesture (a materially bigger feature the web version doesn't even have — it's mouse-hover, not drag), the anchor day just shows as a normal selected day until the second tap lands and the full band appears at once, the same strict-touch-subset trade-off `RatingStars`/`ToggleGroup`/`Calendar` itself already made for other dropped hover/keyboard affordances. `rangeUtils.ts` (`emptyRange`/`orderRange`/`rangeLength`/`isWithinRange`/`isRangeAllowed`/`isRangeComplete`) ports verbatim, zero DOM dependency on the web side already. **Range-band visual has no CSS `::before`/pseudo-element or `color-mix()` to lean on**: reproduced with an absolutely-positioned `View` behind each day's `Pressable`, bleeding half the cell's own 2dp padding into each neighbor via a negative inset (mirrors the web CSS's `inset-inline: calc(gap/-2)` bleed, ported as JS math since RN has no pseudo-elements) so consecutive in-range days read as one continuous stripe under the round day buttons; end caps skip the bleed on their outer side and round that corner instead, so a single-day range (both `rangeStart`/`rangeEnd` on one cell) naturally composes into a full pill from the same two conditionals rather than a separate case. Tint alpha uses the `#RRGGBBAA` hex-suffix substitution `Blockquote`/`Chip` already established for the web's `color-mix()` (`24` ≈ 14% committed, `14` ≈ 8% preview). Confirmed on the iOS Simulator: the band, its rounded end caps, and `Calendar`'s own today-ring all composed correctly together on the first screenshot, no bug found. 17/17 new tests passed on the first run |
| ⬜ | **DateRangePicker** | `Popover` + `CalendarRange` composition — unblocked now that `CalendarRange` has shipped |
...
- **`CalendarRange` shipped (Tier 20 molecule)**: see its own row above for
  the full design writeup. First component in this package to extract a
  `CalendarBase` split out of an already-shipped component's internals
  (`Calendar.tsx`) rather than building fresh — `Calendar`'s own ROADMAP
  note predicted this exact extraction back when it shipped. `Calendar`'s
  full pre-existing test suite (29 tests) passed against the refactor
  unmodified, confirming no behavior regressed. `DateRangePicker` remains
  unbuilt; not picked up in this same turn.
