...
describe('LineChart', () => {
  describe('rendering', () => {
    it('renders without crashing', () => {
      const { container } = render(<LineChart data={DATA} series={SERIES} xAxisKey="day" />);

      expect(container.firstChild).toBeInTheDocument();
    });

    it('applies height to the wrapper div', () => {
      const { container } = render(<LineChart data={DATA} series={SERIES} height={450} />);
  ...
  describe('HTML attribute forwarding', () => {
    it('forwards className to the wrapper div', () => {
      const { container } = render(<LineChart data={DATA} series={SERIES} className="my-line" />);
