...
describe('CloudChart', () => {
  ...
  describe('accessibility', () => {
    ...
    it('uses a custom aria-label when provided', () => {
      const { container } = render(<CloudChart data={DATA} aria-label="Technology popularity" />);
    ...
    it('adds aria-label with value to each word span', () => {
      render(<CloudChart data={DATA} />);
      expect(screen.getByText('TypeScript')).toHaveAttribute('aria-label', 'TypeScript 95');
    });
  });
  ...
  describe('custom colors', () => {
    it('applies per-item color override', () => {
      render(<CloudChart data={[{ text: 'Rust', value: 50, color: '#ff0000' }]} />);
