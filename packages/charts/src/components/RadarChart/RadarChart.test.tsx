...
describe('RadarChart', () => {
  describe('rendering', () => {
    it('renders without crashing', () => {
      const { container } = render(<RadarChart data={DATA} series={SERIES} angleKey="skill" />);

      expect(container.firstChild).toBeInTheDocument();
    });

    it('applies height to the wrapper div', () => {
      const { container } = render(<RadarChart data={DATA} series={SERIES} height={350} />);
  ...
  describe('accessibility', () => {
    ...
    it('falls back to dataKey in aria-label when name is omitted', () => {
      const { container } = render(<RadarChart data={DATA} series={[{ dataKey: 'alice' }]} />);
    ...
    it('uses the custom aria-label when provided', () => {
      ...
      expect(container.querySelector("[role='img']")).toHaveAttribute('aria-label', 'Skill radar');
    });
  });
