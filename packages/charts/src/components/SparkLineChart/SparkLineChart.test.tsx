...
describe("SparkLineChart", () => {
  describe("rendering", () => {
    ...
    it("renders without crashing with object array and dataKey", () => {
      const { container } = render(
        <SparkLineChart data={OBJECTS} dataKey="sessions" />,
      );
      expect(container.firstChild).toBeInTheDocument();
    });
    ...
    it("applies custom height to wrapper", () => {
      const { container } = render(
        <SparkLineChart data={NUMBERS} height={32} />,
      );
  ...
  describe("accessibility", () => {
    it("sets role=img and aria-label when aria-label is provided", () => {
      const { container } = render(
        <SparkLineChart data={NUMBERS} aria-label="Sessions trend" />,
      );
  ...
  describe("HTML attribute forwarding", () => {
    it("forwards className to the wrapper div", () => {
      const { container } = render(
        <SparkLineChart data={NUMBERS} className="my-spark" />,
      );
