...
describe('LineChart', () => {
  describe('rendering', () => {
    it('renders without crashing', async () => {
      await render(withProvider(<LineChart data={DATA} series={SERIES} xAxisKey="day" />));
  ...
  describe('accessibility', () => {
    ...
    it('falls back to dataKey in the accessibility label when name is omitted', async () => {
      await render(
        withProvider(<LineChart data={DATA} series={[{ dataKey: 'cpu' }]} xAxisKey="day" />),
      );
    ...
    it('uses the custom aria-label when provided', async () => {
      await render(
        withProvider(
          <LineChart data={DATA} series={SERIES} xAxisKey="day" aria-label="CPU over time" />,
        ),
      );
