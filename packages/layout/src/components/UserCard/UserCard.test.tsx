...
describe('UserCard', () => {
  ...
  describe('actions', () => {
    ...
    it('calls onClick when an action button is clicked', () => {
      ...
      render(<UserCard name="Ada Lovelace" actions={[{ label: 'View Profile', onClick }]} />);
      fireEvent.click(screen.getByRole('button', { name: 'View Profile' }));
      expect(onClick).toHaveBeenCalledTimes(1);
    });

    it("renders the actions container with role='list'", () => {
      const { container } = render(<UserCard name="Ada Lovelace" actions={defaultActions} />);
  ...
  describe('separator logic', () => {
    it('renders one separator (after header) when there are only non-destructive actions', () => {
      const { container } = render(
        <UserCard
          name="Ada Lovelace"
          actions={[{ label: 'View Profile' }, { label: 'Account Settings' }]}
        />,
      );
    ...
    it('inserts an extra separator before a destructive action that follows non-destructive ones', () => {
      const { container } = render(
        <UserCard
          name="Ada Lovelace"
          actions={[{ label: 'View Profile' }, { label: 'Sign Out', variant: 'destructive' }]}
        />,
      );
    ...
    it('does not insert an extra separator when the first action is destructive', () => {
      const { container } = render(
        <UserCard name="Ada Lovelace" actions={[{ label: 'Sign Out', variant: 'destructive' }]} />,
      );
  ...
  describe('loading', () => {
    ...
    it("renders skeleton when loadingType='skeleton' is explicit", () => {
      const { container } = render(<UserCard name="Ada Lovelace" loading loadingType="skeleton" />);

      expect(container.firstChild).toHaveAttribute('aria-busy', 'true');
    });

    it("renders spinner when loadingType='spinner'", () => {
      const { container } = render(<UserCard name="Ada Lovelace" loading loadingType="spinner" />);
  ...
  describe('HTML attribute forwarding', () => {
    it('forwards className to the root element', () => {
      const { container } = render(<UserCard name="Ada Lovelace" className="custom-card" />);
    ...
    it('merges custom style with the minWidth default', () => {
      const { container } = render(<UserCard name="Ada Lovelace" style={{ padding: '8px' }} />);
