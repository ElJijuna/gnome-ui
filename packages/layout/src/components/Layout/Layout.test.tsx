...
describe('Layout', () => {
  ...
  it('renders all four zones simultaneously', () => {
    render(
      <Layout topBar={<div>Header</div>} sidebar={<nav>Sidebar</nav>} bottomBar={<div>Footer</div>}>
        <main>Content</main>
      </Layout>,
    );
  ...
  it('uses semantic landmark elements for the shell zones', () => {
    const { container } = render(
      <Layout header={<div>Header</div>} sidebar={<div>Navigation</div>} footer={<div>Footer</div>}>
        <p>Content</p>
      </Layout>,
    );
  ...
  it.each(['content', 'page', 'none'] as const)('supports the %s scroll model', (scroll) => {
    const { container } = render(
  ...
  describe('mobile sidebar overlay', () => {
    ...
    it('calls onSidebarOpenChange with backdrop reason when the backdrop is clicked', () => {
      ...
      const { container } = render(
        <Layout sidebar={<nav>Nav</nav>} sidebarOpen onSidebarOpenChange={onOpenChange}>
          <p>Content</p>
        </Layout>,
      );
      const backdrop = container.querySelector("[class*='backdrop']") as HTMLElement;
    ...
    it('moves focus into the sidebar when opened', () => {
      render(
        <Layout sidebar={<button type="button">First sidebar action</button>} sidebarOpen>
    ...
    it.each([
      ...
      'medium',
      'wide',
    ] as const)('applies %s sidebar breakpoint class', (sidebarBreakpoint) => {
      const { container } = render(
