...
function isSidebarOverlay(breakpoint: keyof typeof sidebarBreakpointQuery = 'narrow') {
  return (
    typeof window !== 'undefined' && window.matchMedia(sidebarBreakpointQuery[breakpoint]).matches
  );
}
        ...
        <>
          ...
          <WrapBox childSpacing={24} lineSpacing={24} align="start">
            <Box spacing={8} style={{ flex: '1 1 260px', minWidth: 0 }}>
              <Text variant="caption-heading" color="dim" style={{ paddingLeft: 4 }}>
                Recent
              </Text>
              <BoxedList>
                <ActionRow
  ...
  render: () => (
    ...
    >
      <Layout
        ...
        scroll="content"
        header={
          <ShellAppHeader title="Nested View" actions={<Button variant="flat">Refresh</Button>} />
        }
        footer={
          <StatusBar>
            <Text variant="caption" color="dim">
