...
describe('ActivityFeed', () => {
  ...
  describe('items', () => {
    ...
    it('renders description when provided', () => {
      render(<ActivityFeed items={[makeItem({ description: 'admin@example.com' })]} />);
    ...
    it('renders icon when provided', () => {
      render(<ActivityFeed items={[makeItem({ icon: <span data-testid="icon" /> })]} />);
  ...
  describe('loading', () => {
    ...
    it('does not render item labels when loading', () => {
      render(<ActivityFeed items={[makeItem({ label: 'Hidden label' })]} loading />);
    ...
    it("renders skeleton rows when loadingType='skeleton' is explicit", () => {
      const { container } = render(<ActivityFeed items={[]} loading loadingType="skeleton" />);

      expect(container.querySelector('ul')).toBeInTheDocument();
    });

    it("renders spinner when loadingType='spinner'", () => {
      const { container } = render(<ActivityFeed items={[]} loading loadingType="spinner" />);
  ...
  describe('HTML attribute forwarding', () => {
    it('forwards className to the root element', () => {
      const { container } = render(<ActivityFeed items={[]} className="my-feed" />);

      expect(container.firstChild).toHaveClass('my-feed');
    });

    it('forwards data attributes', () => {
      const { container } = render(<ActivityFeed items={[]} data-testid="feed" />);
