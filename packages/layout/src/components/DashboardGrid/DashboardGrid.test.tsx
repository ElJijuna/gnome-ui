...
describe('DashboardGrid', () => {
  ...
  describe('columns prop', () => {
    ...
    it.each([1, 2, 6, 12] as const)('applies repeat(%s, 1fr) for columns=%s', (cols) => {
  ...
  describe('gap prop', () => {
    it.each([
      ...
      'md',
      'lg',
    ] as const)("sets --dashboard-grid-gap-xs CSS variable for gap='%s'", (gap) => {
    ...
    it('defaults to md gap (gnome-space-3 token)', () => {
      ...
      expect(grid.style.getPropertyValue('--dashboard-grid-gap-xs')).toContain('gnome-space-3');
    });

    it('sets per-breakpoint gap CSS variables for responsive gap', () => {
      const { container } = render(<DashboardGrid gap={{ xs: 'sm', md: 'md', xl: 'lg' }} />);
      const grid = container.firstChild as HTMLElement;
  ...
  describe('DashboardGrid.Item span prop', () => {
    ...
    it.each([2, 3, 6, 12] as const)('sets --item-span-xs to %s for span=%s', (span) => {
      const { container } = render(
        <DashboardGrid>
