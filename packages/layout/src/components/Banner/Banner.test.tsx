...
describe('Banner', () => {
  ...
  describe('action button', () => {
    it('renders the action button when provided', () => {
      render(<Banner action={{ label: 'Reconnect', onClick: vi.fn() }}>Offline</Banner>);
    ...
    it('calls action.onClick when the action button is clicked', async () => {
      ...
      render(<Banner action={{ label: 'Retry', onClick }}>Error</Banner>);
  ...
  describe('type variants', () => {
    it.each([
      ...
      'warning',
      'error',
    ] as const)("applies typeXxx class for type='%s'", (type) => {
  ...
  describe('HTML attribute forwarding', () => {
    ...
    it('forwards data attributes to the root element', () => {
      const { container } = render(<Banner data-testid="the-banner">Msg</Banner>);
