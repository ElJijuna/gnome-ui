...
describe('PageContent', () => {
  ...
  it.each([
    ...
    'normal',
    'spacious',
  ] as const)('applies %s padding class', (padding) => {
    const { container } = render(<PageContent padding={padding}>Documents</PageContent>);

    expect((container.firstChild as HTMLElement).className).toMatch(
      new RegExp(`padding${padding[0].toUpperCase() + padding.slice(1)}`),
    );
  });
