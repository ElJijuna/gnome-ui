...
describe('useNativeEvent', () => {
  ...
  it('always calls the latest handler without re-subscribing', () => {
    ...
    const { rerender } = renderHook(
      ({ handler }: { handler: (p: unknown) => void }) => useNativeEvent('open-modal', handler),
  ...
  it('re-subscribes when the event type changes', () => {
    const { rerender } = renderHook(({ type }: { type: string }) => useNativeEvent(type, vi.fn()), {
  ...
  it('does not re-subscribe when only the handler changes', () => {
    const { rerender } = renderHook(
      ({ handler }: { handler: () => void }) => useNativeEvent('open-modal', handler),
