...
| ✅ | **`TimePicker`** | Hour/minute selection built from paired `SpinButton`s in a `Popover`, with 12/24-hour format support. The columns live in an internal `TimeFields`, shared with `DatePicker`/`DateRangePicker`'s `showTime` footer |
| ✅ | **`CalendarRange`** | Start/end date-range selection sharing `Calendar`'s grid engine (roving tabindex, month/year drill-down, `min`/`max`). Emits through `onChange` **only once both ends have a value** — a first click sets the anchor and paints a live preview band up to the hovered (or keyboard-focused) day, and the second click commits the range; picking backwards reorders the pair instead of failing. `visibleMonths` (default `1`) renders N side-by-side month panels paged as one unit, so a range spanning months is picked without navigating. Adds `minRange`/`maxRange` day limits on top of `min`/`max`, `aria-selected` on every in-range cell plus start/in-range/end suffixes in the day labels, and <kbd>Esc</kbd> to cancel an in-progress range (after leaving the drill-down) |
